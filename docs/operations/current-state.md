...
The canonical non-secret Legendary Poet target remains project `legendary-poet`, Instagram account `17841435926122104`, username `the.legendary.poet`, Facebook Page `1299632476567433`, login mode `facebook`, Graph host `https://graph.facebook.com`. Credentials authenticate this target; they do not redefine it and are not repository state. A fresh local read-only production preflight on `main@ff392b610cb1cdfd6278513a9ee024ca67497c66` succeeded against that exact account/username and returned publishing quota `0/100`; credential validity, quota and duplicate state are point-in-time evidence and must still be re-proved before any later provider write.
