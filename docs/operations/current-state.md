...
### Current checkpoint
The exact repository checkpoint used for this documentation sync is `main` `bf8db5eeee131414d22add2c803be500d896e4c1`. Resolve fresh `main` again before any later operation; this SHA is evidence, not standing authority.
...
- Lord God VK postponed-content wave: #646 added the guarded video/text wall adapter, #652 made equivalent aliases share flood evidence, #653 bound local wall authority to provider-verified token metadata, #654 removed reviewed-but-unpublished historical Telegram material from eligibility, #656 corrected VK API code 9 to credential-global quarantine with explicit `vk flood-reset-all --confirm-code 9` recovery, and #657 made the replacement-lock proof Windows-safe. The current provider-inert candidate is 83 operations: 60 VK videos, 22 published+verified quote posts and one published+verified historical v3 post (`lordchrist-history-spurgeon-down-grade-1887-v3`, Telegram message `1516`). It is not executable authority until the global quarantine is explicitly reopened, a fresh live wall audit is completed, and the candidate is rebuilt from that audit.
...
Live execution is currently fail-closed by one credential-global VK API code-9 quarantine whose durable forensic evidence includes `apps.get`, `groups.getById`, and `wall.get`. `default` and `legendary-poet` share the same exact credential, and local verified metadata records `video,wall,groups`. No automatic TTL is assumed. The only supported reopening command is the explicit credential-wide `vk flood-reset-all --account legendary-poet --confirm-code 9`; per-method cleanup must not be used to reopen the final evidence entry, and direct state editing/bypass remains forbidden. After reopening, run exactly one fresh read-only wall audit for community `60805374`, rebuild the backlog/handoff from that fresh audit, re-prove exact current `main`, and only then execute the rebuilt Wave once with a new journal. Unknown outcome requires reconciliation and no blind replay. The older 91/93-operation handoffs and the historical 83-operation candidate must not be applied.
