...
The upload path does not call `wall.get` and does not build a multi-video batch ahead of the first result. A definite reservation rejection with no returned ticket or binary effect ends that Wave attempt without ambiguity; a later separately invoked Wave may reopen only that known-no-effect state, archives its prior media/intent/error evidence, and reverifies current media before one new reservation attempt. Unknown or remote-bearing states never reopen. Continue only from current `main` and the durable per-source state.
