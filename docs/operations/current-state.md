...
### Current checkpoint
The exact repository checkpoint used for this documentation sync is `main` `026893dd22e45dd246fb8482a945f6c9b5290bce`. Resolve fresh `main` again before any later operation; this SHA is evidence, not standing authority.
...
- Instagram: guarded production publisher, strict ruleset-v3 Reel proof (progressive scan, 4:2:0, closed GOP, faststart/no-edit-list), local resumable MP4 upload with durable child ledger, phase-aware reconciliation, explicit non-retriable 4xx terminalization, secret-safe rupload wire diagnostics, and a provider-inert `validate-local` command sharing the production verifier (#569, #573, #585, #599, #601, #606, #611, #617, #623, #627).
...
Issue #571 / PR #573 adds canonical Reel media compatibility. PR #585 adds direct local-MP4 resumable upload with a hard 1 GB boundary, exact-byte re-proof, durable child state and zero blind replay. PR #599 aligns the binary upload request with Meta's reviewed complete-byte framing. PR #601 / issue #600 adds phase-aware `video_status` diagnostics and terminal upload/processing error handling; PR #606 routes CLI reconciliation through that resumable state model. PR #611 immediately terminalizes definite HTTP 4xx upload rejects that explicitly report `retriable=false` while transport/5xx ambiguity remains fail-closed/no-replay.

PR #617 / issue #614 completes the strict local Reel proof for new resumable attempts: ruleset v3 requires progressive scan, 4:2:0 chroma and deterministic closed-GOP IDR evidence in addition to MOV/MP4, H.264/HEVC, AAC <=48 kHz mono/stereo, FPS, dimensions/aspect, bitrate, duration, size, faststart and no-edit-list checks. Missing or incompatible evidence fails before durable/provider mutation. PR #627 / issue #624 adds `video-manager instagram production validate-local <video_path>`, a credential-free, DB-free and provider-inert operator command that runs that same production verifier and reports exact byte/MP4/media/closed-GOP evidence without creating publication intent.

PR #623 / issue #613 preserves a non-secret resumable request fingerprint and provider correlation on failures while redacting credentials and one-time rupload URI/path fragments. The reviewed request keeps Content-Type absent because Meta does not document a required resumable value; changing it is a protocol change, not an implicit retry. If the rupload/OIL boundary remains unusable, the preferred fallback is the already-supported public-HTTPS `video_url` publisher, but only with a stable direct object URL whose exact content type, byte size and SHA-256 can be re-fetched without redirects before Graph container creation.
...
A fresh local read-only production preflight succeeded against that exact account/username and returned publishing quota `0/100` at the checkpoint used for the canary investigation. Credential validity remains a fresh runtime condition and must be re-proved before later provider work; no token value is repository state.

Provider publications remain **0**. No tested local-resumable attempt through `-04` reached `media_publish`. The reviewed CLEAN `-04` artifact was exact bytes `sha256:93246e03a794e51d8b18b59f3a23cf9617c68e30bc1e723ca3265b914938667e`, size `81833620` bytes, with `ftyp -> moov -> mdat` and no `edts/elst`. Its single authorized write invocation created container `18095416130197538`; the binary upload then returned HTTP 400 `ProcessingFailedError` with `retriable=false`. Read-only reconciliation proved parent `terminal_failure`, child `provider_failed`, provider error `1363008` / `OIL Error[FILE_NOT_FOUND: 6]`, `bytes_transferred=0` and `source_file_size=0`, while Meta's top-level status remained stale `IN_PROGRESS`.

That same zero-byte OIL/FILE_NOT_FOUND phase signature had already been observed on the prior failed canary. The CLEAN remux and complete-byte uploader therefore removed the earlier edit-list and streaming-framing hypotheses without resolving Meta's rupload/OIL ingest failure. Existing `-01` through `-04` publication keys/containers are immutable no-replay evidence and must not be uploaded, published or recreated.

Issue #581 remains open for the first **successful** Legendary Poet canary. There is no authorization for `-05` or any other new Instagram mutation. A future attempt must start from fresh current `main`, use `validate-local` to re-prove the exact media, re-prove target/quota read-only, use a new reviewed publication identity/transport, and receive fresh operation-specific execution authority. A public-HTTPS fallback additionally requires a stable direct object URL on an explicitly allowed host and exact remote-byte verification; a share page, redirector or unproved expiring browser URL is not sufficient.

The handoff remains one-way: reviewed local artifact -> provider-inert `validate-local` / strict Reel proof -> exact manifest/state binding -> read-only target/quota proof -> separately authorized single provider write -> provider-visible reconciliation. No blind retry is allowed from ambiguous or terminal provider state.
...
4. Instagram #577/#600/#605/#610/#613/#614/#624 are complete. #581 remains open for the first successful canary. Never replay `-01`..`-04`. Do not create `-05` without a new reviewed transport/publication identity, provider-inert `validate-local` proof, fresh exact-target/quota proof and fresh operation-specific authority. Prefer the verified public-HTTPS `video_url` fallback if a stable direct object URL can be provisioned and byte-proved; otherwise treat rupload/OIL as externally blocked rather than A/B testing undocumented headers.
