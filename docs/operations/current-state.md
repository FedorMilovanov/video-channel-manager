...
Issue #581 remains open for the first **successful** Legendary Poet canary. A new URL-transport identity is frozen locally as `legendary-poet-yesenin-chto-eto-takoe-url-canary-20260912-01`, bound to account `17841435926122104`, the reviewed CLEAN media, the exact caption recovered from the immutable `-04` ledger (`1432` characters), and `share_to_feed=true`. Its durable content hash is `sha256:38d5a6206b24dcb66daed07cee9f45d6a40eecd71459827108d88effe49f203f`; ledger state is `planned`, attempts `0`, and provider container/media/status are null. A separate read-only media-list checkpoint observed `seen=0`. This planned state is preparation only and does **not** authorize `publish --execute`; a fresh operation-specific authority for this exact key/manifest is still mandatory.
...
4. Instagram #577, #638 and #642 are complete. #581 remains open for the first successful canary. The exact URL-transport canary is already provider-inertly validated and durably `planned`; before any live write, re-prove fresh current `main`, target/quota/duplicate state and remote-media integrity, then require fresh operation-specific authority for exactly `legendary-poet-yesenin-chto-eto-takoe-url-canary-20260912-01`. Never replay `-01`..`-04`; no `-05` resumable attempt is authorized.
