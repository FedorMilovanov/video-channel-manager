...
### Current checkpoint
The exact repository checkpoint used for this documentation sync is `main` `d27998c9ee6f173e78797189711dc1a774b4a609`. Resolve fresh `main` again before any later operation; this SHA is evidence, not standing authority.
...
- Lord God VK postponed-content wave: #646 added the guarded video/text wall adapter, #652 made code-9 circuits credential-scoped, #653 bound local wall authority to provider-verified token metadata, and #654 removed reviewed-but-unpublished historical Telegram material from eligibility. The current provider-inert candidate is 83 operations: 60 VK videos, 22 published+verified quote posts and one published+verified historical v3 post (`lordchrist-history-spurgeon-down-grade-1887-v3`, Telegram message `1516`). It is not executable authority until a fresh live wall audit is completed and the candidate is rebuilt from that audit.
...
Live execution is currently fail-closed by durable credential-scoped VK API code-9 circuits for `groups.getById` and `wall.get`; `apps.get` is also open from the token-capabilities lane. `default` and `legendary-poet` share the same exact credential, and local verified metadata records `video,wall,groups`. No automatic TTL is assumed. Do not bypass or directly edit circuit state, do not apply the 83-operation candidate, and do not reuse the older 91/93-operation handoffs. The next allowed step is one explicit safe-read circuit recovery through the supported operator flow, one fresh read-only wall audit, then a complete backlog/handoff rebuild before any `wall.post`.
