...
### Current checkpoint
The exact repository checkpoint used for this documentation sync is `main` `b9ba536369426823f37df004f3d790d36b3034ce`. Resolve fresh `main` again before any later operation; this SHA is evidence, not standing authority.
...
- Instagram: guarded production publisher, exact target binding, local resumable MP4 upload with durable no-replay state, phase-aware failure reconciliation, immediate terminalization of explicit non-retriable 4xx rejects, ruleset-v3 progressive/4:2:0/closed-GOP proof, secret-safe wire/correlation diagnostics, reviewed public-HTTPS `video_url` fallback, credential-free `validate-local` and `validate-public` boundaries that reuse production verification, plus legacy-Windows Unicode-safe CLI rendering (#569, #573, #585, #592, #599, #601, #606, #611, #617, #623, #627, #637, #644).
...
- Lord God VK postponed-content wave: #646 added the guarded video/text wall adapter, #652 made equivalent aliases share exact-method flood evidence, #653 bound local wall authority to provider-verified token metadata, #654 removed reviewed-but-unpublished historical Telegram material from eligibility, #657 made the replacement-lock proof Windows-safe, and #659 restored exact-method code-9 circuits after reverting the over-broad credential-global interpretation from #656. The current provider-inert candidate is 83 operations: 60 VK videos, 22 published+verified quote posts and one published+verified historical v3 post (`lordchrist-history-spurgeon-down-grade-1887-v3`, Telegram message `1516`). It is not executable authority until each required read method is explicitly recovered and probed, a fresh live wall audit is completed, and the candidate is rebuilt from that audit.
...
## VK / Lord God postponed-content wave
The current implementation on `main` is the guarded Wave path from #646 hardened by #652, #653 and #654. Text and video wall operations bind exactly to `project_key=lord-god-strength`, community `60805374` and owner `-60805374`, use exact source identity, complete published+postponed preflight, deterministic GUIDs, one-dispatch/no-blind-replay mutation semantics and exact postflight/reconciliation.
The latest provider-inert production candidate contains exactly 83 operations: 60 low-view/unposted VK videos, 22 Telegram quote posts whose canonical queue payloads are `published` / `provider_effect=verified`, and one historical v3 post, `lordchrist-history-spurgeon-down-grade-1887-v3`, whose Telegram message `1516`, release binding, revision/post Git blobs and `document_sha256=sha256:387e43eb2b3adbc3f25ee0d41e9e06e55d466bd1802c763d6f19eb134d7bca33` are exact production evidence. The other eight reviewed historical v3 posts are ineligible until durable published+verified evidence exists.
The reproduced candidate digests are operation set `2327edc3e5ef5d8c4a0245b9c2204be92de34574ac8b88292d470de07ca74b2b`, source `ae4ebae4716714f372a20bdd55fbd10e6eb2f43629c653503e5191d6d8490fec`, and plan `b1216c6f9902ebdd59fc39ea8d061d9c061071ac30d82e7bcafe86a9b4218d1b`. These are candidate evidence only: the source VK audit is historical `review_required` evidence and must not be treated as a current absence/duplicate authority.
Live execution is currently fail-closed by exact-method VK API code-9 circuits shared across equivalent aliases of the same token. Current durable evidence includes `groups.getById` and `wall.get`; those methods must be recovered independently. `default` and `legendary-poet` share the same exact credential, and local verified metadata records `video,wall,groups`. No automatic TTL is assumed. Direct flood-state editing/bypass remains forbidden. Recovery is staged: use `vk flood-reset --account legendary-poet --method groups.getById --confirm-code 9`, then exactly one `vk community-probe --account legendary-poet --community 60805374`; only after that succeeds may `wall.get` be reopened and checked with exactly one `vk wall-probe --account legendary-poet --community 60805374`. A failed probe reopens only that method circuit and stops recovery. Only after both probes succeed may one fresh read-only wall audit run; rebuild the backlog/handoff from that fresh audit, re-prove exact current `main`, and only then execute the rebuilt Wave once with a new journal. Unknown outcome requires reconciliation and no blind replay. The older 91/93-operation handoffs and the historical 83-operation candidate must not be applied.
...
The canonical non-secret Legendary Poet target remains project `legendary-poet`, Instagram account `17841435926122104`, username `the.legendary.poet`, Facebook Page `1299632476567433`, login mode `facebook`, Graph host `https://graph.facebook.com`. Credentials authenticate this target; they do not redefine it and are not repository state. A fresh local read-only production preflight using repository checkpoint `ff392b610cb1cdfd6278513a9ee024ca67497c66` succeeded against that exact account/username and returned publishing quota `0/100`; credential validity, quota and duplicate state are point-in-time evidence and must still be re-proved before any later provider write.

One successful Legendary Poet Instagram Reel is now published through the reviewed public-HTTPS `video_url` path. The reviewed CLEAN local MP4 remains `sha256:93246e03a794e51d8b18b59f3a23cf9617c68e30bc1e723ca3265b914938667e`, size `81833620` bytes; it was produced by stream-copy remux with `-use_editlist 0 -movflags +faststart`, and its MP4 structure was verified as `ftyp -> moov -> mdat` with no `edts/elst` edit list. Historical resumable attempts `-01` through `-04` remain immutable no-replay evidence and did not reach `media_publish`.
...
Issue #642 is closed as **completed**. The exact CLEAN bytes are now hosted at the content-addressed GitHub Pages URL `https://fedormilovanov.github.io/media/instagram/legendary-poet/93246e03a794e51d8b18b59f3a23cf9617c68e30bc1e723ca3265b914938667e.mp4`. The Pages deployment for head `5cff5b0262b9dd7101b133c19b1a22b745f19fa9` succeeded; independent HTTP proof returned 200 with no redirect, `Content-Type: video/mp4`, `Content-Length: 81833620`, and a full downloaded SHA-256 equal to the reviewed CLEAN digest. Production `validate-public` then exited 0 with Meta provider calls/writes `none`.

Issue #581 is closed as **completed** after the first successful Legendary Poet production Reel canary. Publication key `legendary-poet-yesenin-chto-eto-takoe-url-canary-20260912-01` was executed exactly once under fresh operation-specific authority after fresh exact-target preflight, duplicate check and remote-byte validation. Durable state is `published`; provider container `18095889581197538`, media `18135337036649890`, provider status `PUBLISHED`; an independent read-only Graph GET confirmed product type `REELS`, media type `VIDEO`, and permalink `https://www.instagram.com/reel/DdMgCixE8Gl/`. The write kill switch was restored to false after the invocation.

A second Legendary Poet Reel, Sergey Yesenin `Хулиган`, was later published exactly once under its own immutable publication key `legendary-poet-yesenin-hooligan-0c1-2tk9aqg-20260912-01`; provider media is `18615357925040636`, permalink `https://www.instagram.com/reel/DdM0JRiE3-M/`, and durable state is `published`. The current YouTube Shorts rollout snapshot contains 62 portrait Shorts; 2 are already published, 6 are held for rights/editorial review, and 54 are scheduled provider-inert at one slot per day starting `2026-09-14T19:00:00+03:00`. Queue SHA-256 is `56f71f2ec0c593448f4560bec4df3a413c5175fb676ead9246f37434a41b303a`. The daily operator is deliberately provider-inert: it prepares, hashes, validates, stages and plans exact manifests, skips durable `published` keys, and fail-closes on any non-final/non-planned ledger state. Canonical rollout notes: [`instagram-daily-reels-rollout.md`](instagram-daily-reels-rollout.md).
...
4. Instagram #577, #581, #638 and #642 are complete. The first Legendary Poet production Reel is published as media `18135337036649890` at `https://www.instagram.com/reel/DdMgCixE8Gl/`. Future Instagram releases require a new immutable publication key/manifest, fresh current-main/target/quota/duplicate/media proof and fresh operation-specific authority. Never replay `-01`..`-04` or reuse the completed canary identity.
...
6. Lord God postponed-content execution is pending only on fresh live wall authority. Do not apply the 83-operation candidate while the required safe-read code-9 circuits are open. After explicit supported circuit recovery, run exactly one fresh read-only audit for community `60805374`, rebuild the backlog/handoff from that fresh audit, and only then consider the rebuilt plan for execution.
7. For provider-visible work, start from fresh current `main`, fresh durable state, exact target identity, immutable operation identity and a new explicit owning authorization scope.
