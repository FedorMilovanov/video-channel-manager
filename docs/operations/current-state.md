...
- Legendary Poet VK long-form: #622 removed the accidental `wall.get` hard gate; #626 completed durable `vk-native-video-wave-v2` identity and resumability; #629 retired the remaining legacy YouTube→VK mutation routes. The supported path is now one YouTube source per run, canonical provider state, community mutex, fresh video-only duplicate check, explicit zero wall-publication flags, exact-ID readiness verification, and no blind mutation retry. Live provider transfer is still pending.
...
The upload path does not call `wall.get` and does not build a multi-video batch ahead of the first result. The historical `sync_youtube_to_vk*`, exact-ID resume and long-form-tail writer routes are retired and are not alternate provider-write paths. Continue only from current `main` and the durable per-source state.
