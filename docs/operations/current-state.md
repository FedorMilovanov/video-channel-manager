...
### Current checkpoint
The exact repository checkpoint used for this documentation sync is `main` `01e3274001f1e8f0c26ce0fcf609b96fef9326b0`. Resolve fresh `main` again before any later operation; this SHA is evidence, not standing authority.
...
- Instagram: guarded production publisher, exact non-secret Legendary Poet target binding, durable local-resumable state/reconciliation, ruleset-v3 Reel proof (progressive scan, 4:2:0 chroma, closed GOP, no edit lists, faststart), Meta-aligned complete-byte framing, immediate terminalization of explicit non-retriable HTTP 4xx rejects, and secret-safe resumable wire diagnostics plus reviewed public-HTTPS fallback contract (#569, #573, #585, #588, #592, #593, #599, #601, #606, #611, #617, #623).
...
Issue #581 remains open for the first **successful** Legendary Poet canary. There is no authorization for `-05` or any other new Instagram mutation. A future attempt must start from fresh current `main`, re-prove the exact media and target/quota read-only, use a new reviewed publication identity/transport, and receive fresh operation-specific execution authority. A public-HTTPS fallback additionally requires a stable direct object URL on an explicitly allowed host and exact remote-byte verification; a share page, redirector or unproved expiring browser URL is not sufficient.

The handoff remains one-way: reviewed local artifact -> strict Reel/MP4 proof -> exact manifest/state binding -> read-only target/quota proof -> separately authorized single provider write -> provider-visible reconciliation. No blind retry is allowed from ambiguous or terminal provider state.
...
4. Instagram #577/#600/#605/#610/#613/#614 are complete. #581 remains open for the first successful canary. Do not replay `-01`..`-04` and do not create `-05` without a new reviewed transport/publication identity, fresh exact-media/target proof and fresh operation-specific authority. Prefer the verified public-HTTPS `video_url` fallback if a stable direct object URL can be provisioned and byte-proved; otherwise treat rupload/OIL as externally blocked rather than A/B testing undocumented headers.
