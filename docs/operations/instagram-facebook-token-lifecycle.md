...
Repository-side credential lifecycle work is complete, and the current Issue #581 readiness checkpoint already has a valid credential path, successful read-only exact-target preflight, an immutable reviewed Reel artifact, a byte-verified public HTTPS object, and a durable provider-inert URL-canary plan. The remaining live boundary is narrower: immediately before execution, re-prove fresh current `main`, the exact planned ledger state, remote-media integrity, target/quota and duplicate state; then obtain fresh operation-specific authority for exactly one `publish --execute` invocation of `legendary-poet-yesenin-chto-eto-takoe-url-canary-20260912-01`, restore the write kill switch to false afterward, and capture provider-safe reconciliation evidence. Until that exact authority exists, no provider mutation is authorized.
