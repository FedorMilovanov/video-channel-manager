...
Repository-side error classification and this runbook do not complete the live rollout. Issue #581 still requires a fresh valid credential, successful read-only preflight, an immutable reviewed Reel artifact/manifest, and separately authorized execution of exactly one canary with provider-safe postflight evidence.
