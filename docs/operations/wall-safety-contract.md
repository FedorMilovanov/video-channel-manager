...
- VK API code `9` (`Flood control`) is a distinct non-auto-retry condition for methods that are called; a known-open `wall.get` circuit blocks postponed-wall execution and reconciliation locally before any `video.get` or other provider request, and native-video upload no longer calls `wall.get` merely to prove that wall publishing was disabled;
