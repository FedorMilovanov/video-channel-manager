...
- VK API code `9` (`Flood control`) is a distinct non-auto-retry condition for methods that are called; flood state is enforced per credential + method across all local aliases sharing the same access token, so an alias cannot bypass another alias's open circuit; the VK schema flag `global: true` classifies code 9 as a common error definition applicable across methods and MUST NOT be interpreted as a persistent credential-wide quarantine after one occurrence; postponed-wall execution checks the required `wall.get` and `wall.post` circuits locally before any `video.get` or other provider request, exact reconciliation checks `wall.get` the same way, and native-video upload no longer calls `wall.get` merely to prove that wall publishing was disabled;
...
- a one-source postponed-video wall attempt may be retried only when the complete prior Wave result and operation journal prove `rejected_before_dispatch`, `retry_safe=true`, and no reconciliation requirement; a legacy result that exactly records the historical `wall.get` API-code-9 preflight rejection is accepted only as a compatibility case with the same complete digest/journal binding; the prior journal is immutable and the next execution uses a fresh deterministic sibling `journal-retry-NNN`;
- live `video.get` / preflight `wall.get` rejections occur before `wall.post` and are classified retry-safe; any `wall.get` failure after `wall.post` dispatch is an unknown mutation outcome that requires reconciliation, never replay;
