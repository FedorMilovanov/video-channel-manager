...
- VK API code `9` (`Flood control`) is a distinct non-auto-retry condition and the current official VK API schema marks it `global: true`; any recorded code-9 entry therefore opens a credential-global quarantine across all local aliases sharing the same access token, while the exact method remains forensic evidence only; postponed-wall execution checks this global quarantine at its wall boundary before any `video.get` or other provider request, exact reconciliation does the same, and native-video upload no longer calls `wall.get` merely to prove that wall publishing was disabled;
