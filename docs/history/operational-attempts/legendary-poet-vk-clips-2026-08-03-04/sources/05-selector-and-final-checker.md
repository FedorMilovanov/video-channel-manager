...
def get_blob(item: dict[str, Any]) -> str:
    return "\n".join(str(item.get(k) or "") for k in ("title", "description", "direct_url", "share_url"))
...
def scan_new_clips(writer: VkVideoWriter) -> list[dict[str, Any]]:
    ...
    for start in range(FIRST_NEW_VIDEO_ID, LAST_NEW_VIDEO_ID + 1, BATCH_SIZE):
        ...
        for item in parse_items(response):
            if item.get("owner_id") == OWNER_ID and item.get("type") == "short_video" and int(item.get("duration") or 0) > 0 and isinstance(item.get("id"), int):
...
def main() -> int:
    ...
    writer = VkVideoWriter(token_store=store, account_alias=ACCOUNT, api_version=settings.vk_api_version)
    ...
    payload = {
        ...
        "matches": matches,
        "missing": missing,
        "extra": [{"vk_clip_id": item_remote_id(clip), "duration": clip.get("duration"), "title": clip.get("title"), "description": clip.get("description"), "url": clip.get("direct_url") or clip.get("share_url"), "views": clip.get("views")} for clip in extra],
    }
    ...
    with csv_path.open("w", encoding="utf-8-sig", newline="") as f:
        ...
        for row in sorted(matches, key=lambda x: int(x["new_vk_clip_id"].split("_")[-1])):
            w.writerow([row["new_vk_clip_id"], row["source_id"], row["source_title"], row["source_duration"], row["new_vk_duration"], row["vk_url"], row["score"], ",".join(row["reasons"])])
    lines = [
        "ФИНАЛЬНАЯ ПРОВЕРКА VK КЛИПОВ",
        "=" * 72,
        f"Новых short_video в диапазоне {FIRST_NEW_VIDEO_ID}-{LAST_NEW_VIDEO_ID}: {len(clips)}",
        f"Сопоставлено с целевой очередью 48: {len(matches)}/48",
        f"Не найдено из 48: {len(missing)}",
        f"Новые short_video без пары: {len(extra)}",
        "",
        "НОМЕРНАЯ ПРОВЕРКА",
        "-" * 72,
    ]
    ...
    lines.extend(["", "СОПОСТАВЛЕННЫЕ КЛИПЫ", "-" * 72])
    for row in sorted(matches, key=lambda x: int(x["new_vk_clip_id"].split("_")[-1])):
        lines.append(f"{row['new_vk_clip_id']} | YT={row['source_id']} | {row['source_duration']}→{row['new_vk_duration']}с | score={row['score']} | {row['source_title']}")
    lines.extend(["", "НЕ ЗАГРУЖЕНО / НЕ ОПОЗНАНО", "-" * 72])
    if missing:
        for row in missing:
            lines.append(f"YT={row['source_id']} | {row['duration']}с | {row['title']}")
    else:
        lines.append("Нет. Все 48 найдены.")
    lines.extend(["", "НОВЫЕ КЛИПЫ БЕЗ ПАРЫ", "-" * 72])
    if extra:
        for clip in extra:
            lines.append(f"{item_remote_id(clip)} | {clip.get('duration')}с | title={clip.get('title')!r} | desc={str(clip.get('description') or '')[:120]!r}")
    else:
        lines.append("Нет.")
    txt_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print("\n".join(lines))
