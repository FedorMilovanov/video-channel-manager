...
_FIXED_CHILDREN: tuple[tuple[str, str], ...] = (
    ...
    ("upload-session", "resumable_upload_session"),
    ("upload", "upload"),
...
def build_release_state(
    *,
    ...
    if not release_plan_sha256.startswith("sha256:") or len(release_plan_sha256) != 71:
        raise YouTubeReleaseStateError("release_plan_sha256 is invalid.")
...
def validate_release_state(state: dict[str, Any]) -> None:
    ...
    for field in ("upload_key_sha256", "release_plan_sha256"):
        digest = state.get(field)
        if not isinstance(digest, str) or not digest.startswith("sha256:") or len(digest) != 71:
            raise YouTubeReleaseStateError(f"Release state {field} is invalid.")
    ...
    for item in children:
        ...
        if not isinstance(runtime, dict):
            raise YouTubeReleaseStateError(f"Release child {child_id} runtime must be an object.")
        attempt_id = item.get("attempt_id")
        if attempt_id is not None and (not isinstance(attempt_id, str) or not attempt_id):
            raise YouTubeReleaseStateError(f"Release child {child_id} attempt_id is invalid.")
...
def prepare_child(
    ...
    *,
    ...
    if attempt_id is not None:
        if not attempt_id:
            raise YouTubeReleaseStateError("attempt_id cannot be blank.")
        child["attempt_id"] = attempt_id
    if runtime_updates:
        child["runtime"].update(copy.deepcopy(runtime_updates))
    stamp = _now(now)
...
def mark_existing_target_absent(
    state: dict[str, Any],
    *,
    evidence: object,
    now: str | None = None,
) -> dict[str, Any]:
    payload = {"mode": "reviewed_existing_target_absence"}
...
def mark_existing_target_adopted(
    ...
    *,
    ...
    session_payload = {"video_id": video_id, "mode": "adopted_existing_no_upload_session"}
    updated = prepare_child(updated, child_id="upload-session", payload=session_payload, now=now)
    updated = transition_child(
        updated,
        child_id="upload-session",
        provider_effect="verified",
        evidence={"adopted_existing_target": True, "session_not_required": True},
        now=now,
    )
