...
from video_channel_manager.platforms.vk.models import VkAccessToken, VkAccount

_ACCOUNT_ALIAS_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$")


class VkAccountNotFoundError(KeyError):
    pass


class VkTokenStore:
    """Stores secret VK user tokens separately from the non-secret account registry."""

    def __init__(self, data_dir: Path) -> None:
        self.data_dir = data_dir
        self.token_dir = data_dir / "secrets" / "vk"
        self.registry_path = data_dir / "vk" / "accounts.json"
    ...
    def load_token(self, alias: str) -> VkAccessToken:
        path = self.token_path(alias)
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
        except FileNotFoundError as exc:
            raise VkAccountNotFoundError(f"VK account token not found: {alias}") from exc
        return VkAccessToken.model_validate(payload)
    ...
    def list_accounts(self) -> list[VkAccount]:
        if not self.registry_path.exists():
            return []
        payload = json.loads(self.registry_path.read_text(encoding="utf-8"))
        if not isinstance(payload, list):
            raise ValueError(f"Invalid VK account registry: {self.registry_path}")
        return sorted((VkAccount.model_validate(item) for item in payload), key=lambda item: item.alias)

    def get_account(self, alias: str) -> VkAccount:
        alias = self.validate_alias(alias)
        for account in self.list_accounts():
            if account.alias == alias:
                return account
        raise VkAccountNotFoundError(f"VK account is not registered: {alias}")

    def save_account(self, account: VkAccount) -> None:
        self.ensure_directories()
