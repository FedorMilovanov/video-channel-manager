...
def _probe_media(ffprobe: str, media_path: Path) -> tuple[int, int, float]:
    ...
    if not video_streams:
        raise MiloviSourceError(f"No video stream in {media_path}")
    width = max(int(item.get("width") or 0) for item in video_streams)
    height = max(int(item.get("height") or 0) for item in video_streams)
...
def prepare_sources(work_dir: Path) -> list[SourceAsset]:
    ...
    assets = [_download_source(yt_dlp, ffprobe, source_id, work_dir / "media") for source_id in ROLL_OUT_IDS]
    write_json_atomic(
        prepared_path,
        {
