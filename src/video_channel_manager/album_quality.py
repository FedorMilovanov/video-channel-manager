...
from video_channel_manager import album_legacy as legacy
...
def quality_master_path(data_dir: Path, album_key: str) -> Path:
    return legacy.album_root(data_dir, album_key) / "quality-masters.json"


def create_quality_master_manifest(manifest: legacy.AlbumManifest) -> QualityMasterManifest:
...
def load_quality_master_manifest(path: Path, manifest: legacy.AlbumManifest) -> QualityMasterManifest:
    try:
    ...
    if (
        ...
        raise legacy.AlbumError("Quality master manifest belongs to a different album/project/channel")
    return quality


def load_or_initialize_quality_master_manifest(path: Path, manifest: legacy.AlbumManifest) -> QualityMasterManifest:
    if path.is_file():
...
def _track_source_identity(track: legacy.AlbumTrack) -> str:
    if track.source_kind is None or track.status != "probed" or track.sha256 is None:
        raise legacy.AlbumError(f"Track {track.ordinal} must be probed before a quality master can be bound")
    payload = {
...
def bind_quality_master(
    ...
    *,
    ...
    if not resolved.is_file():
        raise legacy.AlbumError(f"Quality master does not exist: {resolved}")
    try:
        report = probe_audio_file(resolved, ffprobe=ffprobe, allowed_extensions=_AUDIO_EXTENSIONS)
    except AudioBatchError as exc:
        raise legacy.AlbumError(f"Track {ordinal} quality master probe failed: {exc}") from exc
    if track.sha256 is None:
        raise legacy.AlbumError(f"Track {ordinal} has no probed source SHA-256")
...
def verify_quality_master_entry(
    ...
    *,
    ...
    if not 1 <= entry.ordinal <= manifest.total_tracks:
        raise legacy.AlbumError(f"Quality master references unknown track {entry.ordinal}")
    track = manifest.tracks[entry.ordinal - 1]
    source_identity = _track_source_identity(track)
    if track.sha256 != entry.source_sha256 or source_identity != entry.source_identity_sha256:
        raise legacy.AlbumError(f"Track {entry.ordinal} quality master belongs to stale source evidence")
    path = Path(entry.master_path).expanduser().resolve()
    if not path.is_file():
        raise legacy.AlbumError(f"Track {entry.ordinal} quality master is missing: {path}")
    if verify_bytes and sha256_file(path) != entry.master_sha256:
        raise legacy.AlbumError(f"Track {entry.ordinal} quality master bytes differ from bound SHA-256")
    return path
...
def require_complete_quality_masters(
    ...
    *,
    ...
    if missing:
        raise legacy.AlbumError(f"Final timing/render requires a bound quality master for every track; missing: {missing}")
...
def manifest_with_quality_master_inputs(
    ...
    for track in manifest.tracks:
        ...
        if track.source_kind == "youtube_exact_source":
            ...
            raise legacy.AlbumError(f"Track {track.ordinal} has no source identity")
