...
def validate_spec(spec: dict[str, Any]) -> None:
    ...
    if not isinstance(media_sha, str) or not media_sha.startswith("sha256:") or len(media_sha) != 71:
        raise UploadPlanError("Spec expected_media_sha256 is invalid.")
...
def validate_intent(intent: dict[str, Any]) -> None:
    ...
    media_sha = str(intent.get("media_sha256") or "")
...
def validate_journal(journal: dict[str, Any], *, intent: dict[str, Any]) -> None:
    ...
    for field in ("project_key", "target_channel_id", "media_sha256", "upload_key_sha256"):
        if journal.get(field) != intent.get(field):
