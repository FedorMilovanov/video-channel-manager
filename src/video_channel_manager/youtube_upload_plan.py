...
def validate_spec(spec: dict[str, Any]) -> None:
    ...
    validate_sha256(media_sha, field="Spec expected_media_sha256")
...
def validate_intent(intent: dict[str, Any]) -> None:
    ...
    media_sha = validate_sha256(intent.get("media_sha256"), field="Intent media_sha256")
...
def validate_live_state_evidence(evidence: dict[str, Any]) -> dict[str, str]:
    ...
    media_sha = validate_sha256(video.get("uploaded_media_sha256"), field="Live-state uploaded_media_sha256")
    if not video_id:
...
def adopted_journal(
    ...
    *,
    ...
    validate_sha256(remote_revision, field="Provider remote revision")
    return {
        "schema_name": JOURNAL_SCHEMA,
        "schema_version": JOURNAL_VERSION,
        "updated_at": now or datetime.now(UTC).isoformat(),
...
def require_adoption_allowed(current: dict[str, Any] | None, *, proposed: dict[str, Any]) -> bool:
