...
def _validate_second_pass(value: dict[str, Any], publication_ids: tuple[str, ...]) -> None:
    ...
    for sequence, raw in enumerate(entries, start=1):
        ...
        impact_counts[cast(str, impact)] += 1
...
def load_release(path: Path, root: Path) -> tuple[dict[str, Any], Any, Any, Any, dict[str, Any]]:
    release = _read(path)
    if (
    ...
    manifest_path = _verify_blob(root, cast(dict[str, Any], release["manifest"]), label="historical manifest")
    verification_path = _verify_blob(
        root,
        cast(dict[str, Any], release["second_pass_verification"]),
        label="historical second-pass verification",
    )
    profile_path = _verify_blob(root, cast(dict[str, Any], release["profile"]), label="historical rich profile")
    legacy_profile_path = _verify_blob(
        root,
        cast(dict[str, Any], release["legacy_profile"]),
        label="LordChrist legacy profile",
    )
    target_binding_path = _verify_blob(
        root,
        cast(dict[str, Any], release["target_binding"]),
        label="historical target binding",
    )

    manifest, queue, registry = materialize_historical_bundle(manifest_path)
    ...
    schedule = cast(dict[str, Any], release.get("schedule") or {})
    if (
...
def load_ledger(
    ...
    *,
    ...
    for index, publication_id in enumerate(expected_ids):
        raw = cast(dict[str, Any], entries[publication_id])
        if (
            ...
            or raw.get("provider_effect") not in {"impossible", "verified", "may_exist", "not_dispatched", "confirmed_absent"}
        ):
...
def _entry(ledger: dict[str, Any], publication_id: str) -> dict[str, Any]:
    ...
    try:
        return entries[publication_id]
    except KeyError as exc:
        raise ValueError("historical publication is outside the durable release ledger") from exc
...
def _guard_unresolved(ledger: dict[str, Any]) -> None:
    for raw in cast(dict[str, dict[str, Any]], ledger["entries"]).values():
        if raw["state"] in {"intent", "may_exist", "failed_no_effect"}:
            raise ValueError(f"unresolved historical provider state blocks writer: {raw['publication_id']}={raw['state']}")
...
def guard_state(
    ...
    *,
    ...
    historical_count = sum(
        ...
        and raw.get("provider_effect") == "verified"
        and raw.get("published_at_utc")
        and datetime.fromisoformat(str(raw["published_at_utc"]).replace("Z", "+00:00")).astimezone(MOSCOW).date().isoformat() == today
    )
...
def decide_scheduled(
    ...
    *,
    ...
    freshness = timedelta(minutes=int(cast(dict[str, Any], release["schedule"])["freshness_minutes"]))
    if age < timedelta(0):
    ...
    remaining = sum(1 for item in cast(dict[str, dict[str, Any]], ledger["entries"]).values() if item["state"] == "pending")
    if remaining <= int(release["replenishment_guard_remaining"]) and release.get("successor_cycle_verified") is not True:
...
def coverage_status(release: dict[str, Any], ledger: dict[str, Any]) -> dict[str, Any]:
    entries = cast(dict[str, dict[str, Any]], ledger["entries"])
    pending = [raw["publication_id"] for raw in entries.values() if raw["state"] == "pending"]
    terminal_verified = [
        raw["publication_id"]
        for raw in entries.values()
        if raw["state"] == "published" and raw.get("provider_effect") == "verified"
    ...
    return {
        ...
        "successor_cycle_verified": release.get("successor_cycle_verified") is True,
        "replenishment_warning": len(pending) <= guard_remaining + 2 and release.get("successor_cycle_verified") is not True,
        "exhaustion_block_active": len(pending) <= guard_remaining and release.get("successor_cycle_verified") is not True,
        "provider_write_performed": False,
    }
...
def _require_target(proof: GenericTargetProof, document: TelegramRichMessageDocument, *, now: datetime | None = None) -> None:
    if (
        proof.chat_id,
...
def prepare(
    ...
    *,
    ...
    if mode == "scheduled":
        ...
        publication_id = cast(str, decision.publication_id)
...
def apply(ledger: dict[str, Any], release: dict[str, Any], intent: dict[str, Any], outcome: TelegramRichProviderOutcome) -> dict[str, Any]:
    publication_id = str(intent.get("publication_id") or "")
    raw = _entry(ledger, publication_id)
    if (
        raw["state"] != "intent"
    ...
    if outcome.provider_effect == "verified":
        ...
        raw.update({"state": "may_exist", "provider_effect": "may_exist", "error": outcome.error or "ambiguous provider effect"})
    else:
        raw.update(
            {
...
def parser() -> argparse.ArgumentParser:
    ...
    commands = ("preview", "ensure-ledger", "guard", "coverage", "preflight", "prepare", "send", "apply", "status")
    for name in commands:
        command = sub.add_parser(name)
        command.add_argument("--release", type=Path, required=True)
        command.add_argument("--root", type=Path, default=Path("."))
        if name in {"ensure-ledger", "guard", "coverage", "prepare", "apply", "status"}:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    planned_dates = cast(tuple[str, ...], aux["planned_dates"])
    target_binding_path = cast(Path, aux["target_binding_path"])
    legacy_profile_path = cast(Path, aux["legacy_profile_path"])
    if args.cmd == "preview":
    ...
    if args.cmd == "guard":
        print(json.dumps(guard_state(args.root, args.state_root, release, cast(dict[str, Any], ledger), legacy_profile_path), ensure_ascii=False))
        return 0
    if args.cmd == "coverage":
        print(json.dumps(coverage_status(release, cast(dict[str, Any], ledger)), ensure_ascii=False))
        return 0
    ...
    if args.cmd == "prepare":
        ...
        print(json.dumps({"prepared": True, "publication_id": intent["publication_id"], "reason": reason}, ensure_ascii=False))
        return 0
    if args.cmd == "send":
        outcome = send(
            args.root,
    ...
    if args.cmd == "apply":
        ...
        updated = apply(cast(dict[str, Any], ledger), release, _read(args.intent), outcome)
        _write(args.ledger, updated)
        print(json.dumps(_entry(updated, str(outcome.publication_id)), ensure_ascii=False))
        return 0
    if args.cmd == "status":
        print(json.dumps(cast(dict[str, Any], ledger), ensure_ascii=False))
        return 0
