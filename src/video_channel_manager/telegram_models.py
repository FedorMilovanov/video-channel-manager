...
class LedgerEntry(BaseModel):
    ...
    def validate_state_evidence(self) -> "LedgerEntry":
        if self.state == "published":
            ...
            if not self.actual_chat_username or not self.message_url:
                raise ValueError("published entries require exact public message identity")
            ...
            if not all(
                (
                    ...
                    self.actual_chat_id,
                    self.actual_chat_username,
                    self.bot_id,
                    self.bot_username,
                )
            ):
                raise ValueError(f"{self.state} entries require exact execution and target provenance")
