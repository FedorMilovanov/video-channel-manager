...
from video_channel_manager import svodka_rich_successor as previous
from video_channel_manager.svodka_rich_loader import load_svodka_rich_article
from video_channel_manager.telegram_channel_profile import load_channel_profile
from video_channel_manager.telegram_rich_models import RichBlockHeading
...
RELEASE_ID = "svodka-rich-v3-finalizer-2026-08"
...
def load_release(path: Path, root: Path) -> dict[str, Any]:
    ...
    if (
        release.get("schema_name") != "video-channel-manager.svodka-rich-successor-v3-release"
        ...
        raise ValueError("invalid Svodka v3 finalizer release header")

    items = release.get("items")
    if not isinstance(items, list) or len(items) != EXPECTED_ITEM_COUNT:
        raise ValueError("Svodka v3 finalizer release must contain exactly one item")
    item = items[0]
    if (
        item.get("sequence") != 1
        ...
        raise ValueError("Svodka v3 finalizer item identity is invalid")

    scheduled = datetime.fromisoformat(str(item["scheduled_at"]))
    if scheduled.utcoffset() != timedelta(hours=3):
        raise ValueError("Svodka v3 finalizer schedule must use UTC+03:00")
...
def _canonicalize_plain_heading_runs(article: Any) -> Any:
    """Use Telegram's observed canonical shape for plain-string heading RichText.

    Message 28 proved that Telegram coalesces adjacent plain string fragments
    such as ["1️⃣", " ", "Heading"] into one returned RichText string. Entity
    dictionaries are never collapsed here; only heading tuples made entirely of
    plain strings are joined before the exact request document is frozen.
    """
...
def build_document(root: Path, release: dict[str, Any], item: dict[str, Any]) -> tuple[Any, Any, Any]:
    ...
    article = _canonicalize_plain_heading_runs(article)
...
def load_ledger(path: Path, release: dict[str, Any], *, create: bool = False) -> dict[str, Any]:
    ...
    if ledger.get("release_id") != RELEASE_ID or ledger.get("release_sha256") != release_digest(release):
        raise ValueError("Svodka v3 ledger is bound to another release")
    ...
    if not isinstance(entries, dict) or list(entries) != expected:
        raise ValueError("Svodka v3 ledger entries differ from release")
    return ledger
...
def select(
    ...
    if entry["state"] != "pending":
        raise ValueError(f"unexpected Svodka v3 state: {entry['state']}")
...
def _item(release: dict[str, Any], publication_id: str) -> dict[str, Any]:
    ...
    if item.get("publication_id") != publication_id:
        raise ValueError(f"unknown Svodka v3 publication id: {publication_id}")
    return item
...
def media_proof(
    ...
    # Reuse the reviewed v2 HTTPS reader and byte/signature proof. Its evidence
    # contract is text-agnostic; release and publication digests are supplied by
    # this v3 release object and remain exact.
    ...
    reader = previous._SuccessorMediaReader()
            ...
            evidence.append(
                {
                    ...
                    "content_sha256": "sha256:" + __import__("hashlib").sha256(content).hexdigest(),
                }
            )
    finally:
        reader.close()
...
def prepare(
    ...
    if selected is None:
        raise ValueError("no eligible Svodka v3 publication")
    item, mode = selected
    ...
    if (
        ...
        raise ValueError("Svodka v3 intent inputs differ from exact release")
...
def send(
    ...
    if (
        ...
        raise ValueError("Svodka v3 durable intent/media recheck mismatch")
    ...
    if document.document_sha256 != intent.get("document_sha256") or render.render_sha256 != intent.get("render_sha256"):
        raise ValueError("Svodka v3 document changed after durable intent")
    ...
    if legacy._sha(proof.model_dump(mode="json")) != intent.get("target_proof_sha256"):
        raise ValueError("Svodka v3 target proof changed after durable intent")
