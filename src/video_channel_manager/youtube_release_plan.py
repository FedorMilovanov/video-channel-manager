...
def validate_absence_evidence(evidence: dict[str, Any], *, plan: dict[str, Any]) -> str:
    ...
    if (
        evidence.get("schema_name") != ABSENCE_EVIDENCE_SCHEMA
        or evidence.get("schema_version") != ABSENCE_EVIDENCE_VERSION
    ):
...
def validate_execution_approval(
    ...
    *,
    ...
    validate_release_plan(plan, verify_files=False)
    if (
        approval.get("schema_name") != EXECUTION_APPROVAL_SCHEMA
        or approval.get("schema_version") != EXECUTION_APPROVAL_VERSION
    ):
