...
def test_fet_verifier_pins_exact_contents_across_zip_packaging() -> None:
    text = _VERIFIER.read_text(encoding="utf-8")

    assert "sha256:0f8020fd76456f8b6490e17e2142d46ca8f18f397ded400c3c093bbf719539f5" in text
    ...
    assert "Reviewed member SHA-256 mismatch" in text
    assert "Manifest file records differ from reviewed contents" in text
    ...
    assert '"artifact_review": "exact_independently_reviewed_contents"' in text
    ...
    assert '"exact_member_hashes_verified": True' in text
