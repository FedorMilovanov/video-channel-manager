...
CHANNEL_PROFILE = ROOT / "content/telegram/channels/lordchrist.json"
...
def _cross_track_paths(tmp_path: Path) -> tuple[Path, Path, Path]:
    predecessor = load_queue(PREDECESSOR_QUEUE)
    predecessor_ledger_path = tmp_path / "publication-ledger.json"
    _write_ledger(predecessor_ledger_path, initialize_ledger(predecessor))
    ...
    return predecessor_ledger_path, research_ledger_path, successor_ledger_path


def test_successor_unresolved_effect_blocks_every_lordchrist_writer(
    tmp_path: Path,
    ...
    predecessor_ledger_path, research_ledger_path, successor_ledger_path = _cross_track_paths(tmp_path)

    runtime = _build_runtime()
    successor = initialize_ledger(runtime)  # type: ignore[arg-type]
    post = runtime.posts[0]
    attempted = datetime(2026, 9, 10, 8, 0, tzinfo=UTC)
    successor.entries[post.publication_id] = LedgerEntry(
    ...
    _write_ledger(successor_ledger_path, successor)

    with pytest.raises(ValueError, match=r"successor=.*lordchrist-successor-"):
        require_no_cross_track_unresolved_effects(
            profile_path=CHANNEL_PROFILE,
            legacy_queue_path=PREDECESSOR_QUEUE,
            legacy_ledger_path=predecessor_ledger_path,
            research_ledger_path=research_ledger_path,
        )


def test_partial_successor_ledger_is_fail_closed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("LORDCHRIST_PREDECESSOR_QUEUE_PATH", raising=False)
    ...
    runtime = _build_runtime()
    successor = initialize_ledger(runtime)  # type: ignore[arg-type]
    successor.entries.pop(runtime.posts[-1].publication_id)
    _write_ledger(successor_ledger_path, successor)

    with pytest.raises(ValueError, match="successor ledger coverage differs"):
        require_no_cross_track_unresolved_effects(
            profile_path=CHANNEL_PROFILE,
            legacy_queue_path=PREDECESSOR_QUEUE,
            legacy_ledger_path=predecessor_ledger_path,
