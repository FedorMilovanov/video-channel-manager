...
def _lord_god_payload() -> dict[str, object]:
    ...
    # The example's poet playlist belongs to another project. Keep only
    # project links plus the independently sourced primary text.
...
def test_unknown_canonical_channel_requires_explicit_project_key() -> None:
    ...
    errors = validate_content_record(payload)
    assert "canonical content requires project_key when channel_id is not registered" in errors
