...
from lord_god_article_wave_v3 import photo_wave_v4 as photo_v4  # noqa: E402
...
def test_photo_v4_builds_new_ids_and_future_schedule() -> None:
    # Importing v5 intentionally replaces the active runtime identity. The
    # historical v4 implementation remains covered through its base mechanics.
    policy = photo_v4.build_photo_policy(ROOT)

    assert policy["decision_set_id"] == photo_v5.PHOTO_DECISION_SET_ID
...
def test_photo_v5_builds_fresh_ids_guids_and_contract() -> None:
    policy = photo_v5.build_photo_policy(ROOT)
...
def test_photo_v5_journal_is_contract_isolated(tmp_path: Path) -> None:
    policy = photo_v5.build_photo_policy(ROOT)
    journal_path = tmp_path / "photo-journal-v4.json"
    old = {
        "schema_name": "video-manager.vk-lord-god-article-photo-wave-journal",
        "schema_version": 4,
        "decision_set_id": "lord-god-article-photo-wave-v4-202608",
    ...
    with pytest.raises(RuntimeError, match="another execution contract"):
        photo_v5.load_photo_journal(journal_path, policy)


def test_photo_v5_fresh_journal_has_no_imported_operations() -> None:
    policy = photo_v5.build_photo_policy(ROOT)
    journal = photo_v5.fresh_photo_journal(policy)
...
def test_active_runner_has_no_parsed_link_call() -> None:
    ...
    assert "photo_wave_v5.py" in runner
