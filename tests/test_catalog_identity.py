...
def _source(*, memberships: list[tuple[str, str, int]] | None = None) -> AuditPackage:
    return _audit(
        ...
        memberships=memberships or [("playlist-1", "yt-1", 0), ("playlist-1", "yt-2", 1)],
    )
...
def test_reviewed_mapping_uses_exact_id_and_ignores_position_order() -> None:
    source = _source(memberships=[("playlist-1", "yt-2", 50), ("playlist-1", "yt-1", 5)])
    target = _target(memberships=[("album-1", "vk-1", 100), ("album-1", "vk-2", 1)])
...
def test_reviewed_mapping_records_missing_and_extra_semantic_membership() -> None:
    target = _target(memberships=[("album-1", "vk-1", 0), ("album-1", "vk-extra", 1)])
...
def test_duplicate_canonical_album_titles_are_conflict() -> None:
    ...
    assert [item.remote_id for item in decision.candidate_target_refs] == ["album-1", "album-2"]
...
def test_unknown_and_reused_reviewed_collection_ids_fail_closed() -> None:
    ...
    with pytest.raises(ValueError, match="reuse target collection"):
        build_catalog_identity_evidence(
            ...
            project_key="legendary-poet",
            video_mapping={},
            reviewed_collection_mappings={"playlist-1": "album-1", "playlist-2": "album-1"},
        )
