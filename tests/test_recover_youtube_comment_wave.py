...
def test_strict_coverage_requires_exactly_one_owned_comment_on_every_video() -> None:
...
def test_strict_coverage_rejects_duplicate_channel_comments() -> None:
    audit = _closed_audit()
    videos = audit["videos"]
    ...
    with pytest.raises(ValueError, match="duplicate channel-authored comments"):
...
def test_strict_coverage_rejects_any_remaining_tail() -> None:
    ...
    with pytest.raises(ValueError, match="without exactly one"):
        strict_coverage_summary(audit, expected_channel="channel-1")
