...
- The exact feed identity is `milovi-feed-YYYYMMDD-NNN`. Runtime release, exactly one content binding, durable ledger, execution authority and channel-wide index must all bind that exact identity and exact payload digest.
- `sendPhoto` uses the exact media/source/transport binding and deterministic JPEG materialization path. `sendMessage` uses an exact candidate/text SHA-256 binding and must not be forced through fake media.
- `sendVideo` is allowed only when the exact `<id>-video.json` binding points to one of the 16 accepted MP4 identities in `content/telegram/milovi-cake/accepted-video-reservoir-2026-08.json`, backed by content-addressed branch `agent/milovi-video-accepted-73c578eff825` and evidence digest `sha256:73c578eff82563300c463361bd3998caeba8a083ce0de4ed29cc271617dfd6ae`. Do not accept arbitrary local/video URLs, re-transcode at publish time, fall back to `sendDocument`, or treat artifact readiness as execution authority.
- Media, message and video bindings are mutually exclusive for one publication. A bundle with more than one binding kind fails closed.
- Release/content authorization and execution/provider authorization are separate gates. Old bootstrap or canary authorization, credentials, automation, accepted video artifacts, a green workflow, or a profile with technical write capability never supplies fresh execution authority.
