#!/usr/bin/env python3
"""Build a signed, title-only VK editorial wave from a fresh VK snapshot."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", type=Path, default=Path("data/reports/vk-editorial-title-wave.json"))
    parser.add_argument("--report", type=Path, default=Path("data/reports/vk-editorial-title-wave.md"))
    return parser
...
def _report(plan: dict[str, Any]) -> str:
    ...
    for operation in plan["video_text_operations"]:
        lines.append(
            f"- `{operation['target_video_id']}` ({operation['duration_seconds']} sec): "
            f"`{operation['before_title']}` → `{operation['after_title']}`"
        )
    lines.extend(["", "## Review only — excluded from this wave", ""])
...
def main() -> int:
    ...
    plan = build_vk_editorial_title_wave(_load_audit(args.target), _load_policy(args.policy_json))
    _atomic_write(args.output, json.dumps(plan, ensure_ascii=False, indent=2) + "\n")
    _atomic_write(args.report, _report(plan) + "\n")
    summary = plan["summary"]
    print(
        "VK editorial title wave built:\n"
        f"  operation scope: {plan['operation_scope']}\n"
        f"  component scope: {plan['component_scope']}\n"
        f"  videos: {summary['videos_in_snapshot']}\n"
