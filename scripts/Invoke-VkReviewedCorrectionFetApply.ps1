...
function Expand-BundleEntry {
    ...
    try {
        ...
        if ($null -eq $Entry) {
            throw "В reviewed Fet dry-run ZIP не найден обязательный файл: $Name"
        }
...
function Read-Count {
    ...
    if (-not $Match.Success) {
        throw "Не удалось прочитать '$Label' из live Fet preflight."
    }
...
function Write-Handoff {
    ...
    $Readme = @"
VK REVIEWED CORRECTION P1 — ФЕТ — APPLY
...
- точная датировка 1850 года;
- отделение факта от биографического фона и исследовательской гипотезы;
- исправление утверждений о Марии Лазич, поздней любовной лирике и последних часах Фета;
- удаление оборванного фрагмента footer.
...
try {
    ...
    if (-not $Execute) {
        throw "Этот helper выполняет только ранее проверенный Fet correction ZIP. Добавьте явный флаг -Execute."
    }
    ...
    if ([string]::IsNullOrWhiteSpace($ReviewedDryRunBundle)) {
        $Latest = Get-ChildItem `
            ...
            -File `
            -Filter "vk-reviewed-correction-p1-fet-dry-run-*.zip" |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1
        if ($null -eq $Latest) {
            throw "Не найден reviewed Fet correction dry-run ZIP."
        }
        $ReviewedDryRunBundle = $Latest.FullName
    }
    $ResolvedDryRun = (Resolve-Path -LiteralPath $ReviewedDryRunBundle).Path

    Write-Host "Независимо проверяется reviewed Fet correction dry-run ZIP..." -ForegroundColor Yellow
    & py -3.11 -X utf8 .\scripts\verify_vk_reviewed_correction_fet_dry_run.py `
        "$ResolvedDryRun" `
        --json-output "$DryRunVerification"
    if ($LASTEXITCODE -ne 0) {
        throw "Reviewed Fet correction dry-run ZIP не прошёл независимую проверку."
    }
    ...
    if ([string]$DryRunReport.status -ne "verified_dry_run" -or
        ...
        [int]$DryRunReport.remote_writes -ne 0) {
        throw "Fet dry-run verification report имеет неожиданный статус или scope."
    }
    $VerifiedIds = @($DryRunReport.target_video_ids | ForEach-Object { [string]$_ } | Sort-Object)
    if (($VerifiedIds -join "|") -cne ($ExpectedIds -join "|")) {
        throw "Независимый Fet verifier подтвердил другой набор video IDs."
    }
    ...
    if ([string]$PlanJson.plan_sha256 -cne [string]$DryRunReport.plan_sha256) {
        throw "Извлечённый Fet plan.json отличается от независимо проверенного плана."
    }
    if ([string]$PlanJson.decision_set_id -cne $ExpectedDecisionSet) {
        throw "Извлечённый plan.json принадлежит другому decision set."
    }
    if ([int]$PlanJson.summary.descriptions_to_update -ne $ExpectedCount -or
        ...
        throw "Извлечённый Fet plan.json имеет неожиданный scope."
    }
    $PlanIds = @(
    ...
    if (($PlanIds -join "|") -cne ($ExpectedIds -join "|")) {
        throw "Извлечённый Fet plan.json содержит неожиданные video IDs."
    }
    ...
    if (-not $PreflightSucceeded) {
        throw "Live Fet correction preflight не завершился после $ReadRetryAttempts попыток."
    }
    ...
    Write-Host ""
    Write-Host "ПРОВЕРЕННЫЙ FET CORRECTION-ПЛАН ДОПУЩЕН К EXECUTE" -ForegroundColor Green
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Reviewed Fet correction execute завершился с ошибкой."
    }
    if (-not (Test-Path -LiteralPath $ResultPath -PathType Leaf)) {
        throw "Fet execute не создал result journal."
    }
    ...
    if ($BadStatuses.Count -gt 0) {
        throw "Fet result journal содержит неподтверждённый статус операции."
    }
    $RemoteWrites = @(
        $ResultOperations |
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Финальный read-only VK scan после Fet correction завершился ошибкой."
    }

    $RunStatus = "completed"
    Write-Handoff

    & py -3.11 -X utf8 .\scripts\verify_vk_reviewed_correction_fet_apply_bundle.py `
        "$ZipPath" `
        --json-output "$FinalVerification"
    if ($LASTEXITCODE -ne 0) {
        throw "Fet correction apply ZIP не прошёл независимую postflight-проверку."
    }
    Copy-Artifact $FinalVerification "05-independent-verification.json"
    Write-Handoff

    & py -3.11 -X utf8 .\scripts\verify_vk_reviewed_correction_fet_apply_bundle.py "$ZipPath"
    if ($LASTEXITCODE -ne 0) {
        throw "Финальный Fet ZIP с embedded verification не прошёл повторную проверку."
    }
    $HandoffVerified = $true

    Write-Host ""
    Write-Host "VK REVIEWED CORRECTION P1 FET ЗАВЕРШЕНА И POSTFLIGHT-ПОДТВЕРЖДЕНА" -ForegroundColor Green
    ...
    try {
        if (-not $HandoffVerified) {
            Write-Handoff
            Write-Host ""
            Write-Host "Создан диагностический Fet apply ZIP:" -ForegroundColor Yellow
            Write-Host $ZipPath -ForegroundColor Cyan
        }
