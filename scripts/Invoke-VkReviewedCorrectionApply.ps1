...
$BundleName = "vk-reviewed-correction-p1-apply-$Stamp"
...
$DryRunManifest = Join-Path $TempDir "dry-run-manifest.json"
...
function Expand-BundleEntry {
    ...
    try {
        ...
        if ($null -eq $Entry) {
            throw "В reviewed dry-run ZIP не найден обязательный файл: $Name"
        }
...
function Write-Handoff {
    ...
    $Readme = @"
VK REVIEWED CORRECTION P1 — APPLY
...
Разрешённый scope:
- ровно три описания «Исповедь Самоубийцы»;
- академическая датировка 1913–1915 вместо 1912;
- духовный вывод в соответствии со стандартами The Legendary Poet и Research.

Заморожено:
- все названия;
- остальные 108 описаний;
- альбомы и их названия;
- 294 memberships;
- состав из 111 видео;
- ссылки, хэштеги, footer и текст стихотворения.

Этот ZIP содержит reviewed dry-run, свежий preflight, result journal,
...
try {
    ...
    if (-not $Execute) {
        throw "Этот helper выполняет только ранее проверенный correction ZIP. Добавьте явный флаг -Execute."
    }
    ...
    if ([string]::IsNullOrWhiteSpace($ReviewedDryRunBundle)) {
        $Latest = Get-ChildItem `
            ...
            -File `
            -Filter "vk-reviewed-correction-p1-dry-run-*.zip" |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1
        if ($null -eq $Latest) {
            throw "Не найден reviewed correction dry-run ZIP."
        }
        $ReviewedDryRunBundle = $Latest.FullName
    }
    $ResolvedDryRun = (Resolve-Path -LiteralPath $ReviewedDryRunBundle).Path

    Write-Host "Независимо проверяется reviewed correction dry-run ZIP..." -ForegroundColor Yellow
    & py -3.11 -X utf8 .\scripts\verify_vk_reviewed_correction_dry_run.py `
        "$ResolvedDryRun" `
        --json-output "$DryRunVerification"
    if ($LASTEXITCODE -ne 0) {
        throw "Reviewed correction dry-run ZIP не прошёл независимую проверку."
    }
    ...
    if ([string]$DryRunReport.status -ne "verified_dry_run" -or
        ...
        throw "Dry-run verification report имеет неожиданный статус или scope."
    }

    Expand-BundleEntry $ResolvedDryRun "manifest.json" $DryRunManifest
    ...
    if ([string]$PlanJson.plan_sha256 -cne [string]$DryRunReport.plan_sha256) {
        throw "Извлечённый plan.json отличается от независимо проверенного плана."
    }
    ...
    for ($Attempt = 1; $Attempt -le $ReadRetryAttempts; $Attempt++) {
        ...
        if ($Text -notmatch "VK API 204 in video\.get: Access denied") {
            throw "Live correction preflight завершился с кодом $ExitCode."
        }
    ...
    if (-not $PreflightSucceeded) {
        throw "Live correction preflight не завершился после $ReadRetryAttempts попыток."
    }
    ...
    Write-Host ""
    Write-Host "ПРОВЕРЕННЫЙ CORRECTION-ПЛАН ДОПУЩЕН К EXECUTE" -ForegroundColor Green
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Reviewed correction execute завершился с ошибкой."
    }
    if (-not (Test-Path -LiteralPath $ResultPath -PathType Leaf)) {
        throw "Execute не создал result journal."
    }
    ...
    if ([string]$ResultJson.status -ne "completed" -or
        ...
        throw "Result journal не подтверждает завершение точного reviewed plan."
    }
    $ResultOperations = @($ResultJson.operations)
    if ($ResultOperations.Count -ne $ExpectedCount) {
        throw "Result journal содержит неожиданное число операций."
    }
    $BadStatuses = @(
        $ResultOperations |
    ...
    if ($BadStatuses.Count -gt 0) {
        throw "Result journal содержит неподтверждённый статус операции."
    }
    $RemoteWrites = @(
        $ResultOperations |
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Correction apply ZIP не прошёл независимую postflight-проверку."
    }
    Copy-Artifact $FinalVerification "05-independent-verification.json"
    Write-Handoff

    & py -3.11 -X utf8 .\scripts\verify_vk_reviewed_correction_apply_bundle.py "$ZipPath"
    if ($LASTEXITCODE -ne 0) {
        throw "Финальный ZIP с embedded verification не прошёл повторную проверку."
    }
    $HandoffVerified = $true

    Write-Host ""
    Write-Host "VK REVIEWED CORRECTION P1 ЗАВЕРШЕНА И POSTFLIGHT-ПОДТВЕРЖДЕНА" -ForegroundColor Green
