...
$Decisions = Join-Path $Repo "content\policies\vk-reviewed-corrections-p1-blok-night-20260728.json"
$Stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$BundleName = "vk-reviewed-correction-p1-blok-dry-run-$Stamp"
...
$Plan = Join-Path $Reports "vk-reviewed-correction-p1-blok-$Stamp.json"
$Report = Join-Path $Reports "vk-reviewed-correction-p1-blok-$Stamp.md"
$HtmlReport = Join-Path $Reports "vk-reviewed-correction-p1-blok-$Stamp.html"
...
function Write-Handoff {
    ...
    $Readme = @"
VK REVIEWED CORRECTION P1 — АЛЕКСАНДР БЛОК
...
- точная передача воспоминаний В. В. Гиппиуса;
- разграничение трёх документированных версий прототипа аптеки;
- отделение факта о цикле от литературной интерпретации;
- исправление хронологии разрешения на лечение за границей;
- удаление недоказуемого абсолютного суперлатива.
...
"@
    ...
    if ($RunStatus -eq "completed") {
        Write-Host "ГОТОВ VERIFIED REVIEWED CORRECTION P1 BLOCK DRY-RUN ZIP:" -ForegroundColor Green
    }
    else {
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($ApplyBundle)) {
        $ApplyBundle = Get-LatestFile `
            "vk-reviewed-correction-p1-fet-apply-*.zip" `
            "Не найден independently verified Fet apply-ZIP."
    }
    ...
    Write-Host "Независимо проверяется source Fet apply-ZIP..." -ForegroundColor Yellow
    & py -3.11 -X utf8 .\scripts\verify_vk_reviewed_correction_fet_apply_bundle.py `
        "$ApplyBundle" `
        --json-output "$SourceApplyVerification"
    if ($LASTEXITCODE -ne 0) {
        throw "Source Fet apply-ZIP не прошёл независимую проверку."
    }
    ...
    if ([string]$SourceApplyReport.status -ne "verified_completed" -or
        [int]$SourceApplyReport.operations -ne $ExpectedCount -or
        [int]$SourceApplyReport.remote_writes -ne $ExpectedCount -or
        ...
        throw "Source Fet apply verification имеет неожиданный статус или scope."
    }
    ...
    if ($BuildExitCode -ne 0) {
        throw "Не удалось построить Block reviewed correction plan. Подробности сохранены в 00-build.txt.`n$BuildText"
    }
    ...
    if ([string]$PlanJson.component_scope -ne "descriptions_only" -or
        ...
        throw "Block correction plan имеет неожиданный scope."
    }
    ...
    if ([int]$PlanJson.summary.descriptions_to_update -ne $ExpectedCount -or
        ...
        throw "Ожидалось ровно два descriptions-only исправления Блока."
    }
    ...
    if (($ActualIds -join "|") -cne ($ExpectedIds -join "|")) {
        throw "Block correction plan содержит неожиданные video IDs."
    }
    ...
    if ($TooLong.Count -gt 0) {
        throw "Block correction plan содержит описание длиннее 5000 символов."
    }
    ...
    if ($ExitCode -ne 0) {
        throw "Live Block correction preflight завершился с кодом $ExitCode."
    }
    ...
    if ($Ready + $AlreadyApplied -ne $ExpectedCount -or $Conflicts -ne 0) {
        throw "Unexpected Block correction preflight: ready=$Ready already=$AlreadyApplied conflicts=$Conflicts"
    }
